...
public class PostgresEventSourcePlugin implements EventSourcePlugin, EventStoreReader {

    private volatile PgEventStoreReader reader;
    ...
    public void initialize(String instanceId, Map<String, Object> config) {
        ...
        this.reader = new PgEventStoreReader(new PgConfig(
    ...
    public EventSourceCapabilities capabilities() {
        return new EventSourceCapabilities(true, true, true, true,
                Set.of("aggregate_id", "aggregate_type", "event_type", "timestamp"));
    }

    @Override
    public EventQueryResult query(EventQuery query) {
        PgEventStoreReader activeReader = requireReader();
        if (query.type() == EventQuery.QueryType.TIMELINE) {
            List<StoredEvent> events;
            if (query.cursor() != null && !query.cursor().isBlank()) {
                long afterSequence = Long.parseLong(query.cursor());
                events = activeReader.getEventsAfterSequence(query.aggregateId(), afterSequence, query.limit() + 1);
            } else {
                events = activeReader.getEvents(query.aggregateId(), query.limit() + 1, 0);
            }
        ...
        List<Event> events = ids.stream()
                .map(id -> activeReader.getEvents(id, 1, 0))
                .filter(list -> !list.isEmpty())
                .map(list -> toSpiEvent(list.get(0), query.fields()))
                .toList();
    ...
    @Override
    public void close() {
        PgEventStoreReader activeReader = this.reader;
        this.reader = null;
    ...
    private PgEventStoreReader requireReader() {
        PgEventStoreReader activeReader = reader;
        if (activeReader == null) {
            throw new IllegalStateException("Postgres plugin is not initialized");
        }
        return activeReader;
    }
    ...
    private static Event toSpiEvent(StoredEvent event, EventQuery.Fields fields) {
        return new Event(
                ...
                event.eventType(),
                fields == EventQuery.Fields.METADATA ? emptyObject() : parseJson(event.payload()),
    ...
    private static JsonNode parseJson(String json) {
        try {
            return JsonUtil.mapper().readTree(json == null || json.isBlank() ? "{}" : json);
        } catch (Exception e) {
            ObjectNode fallback = JsonUtil.mapper().createObjectNode();
    ...
    private static String requireString(Map<String, Object> config, String key) {
        ...
        if (value == null || value.toString().isBlank()) {
            throw new IllegalArgumentException("Missing required postgres config: " + key);
        }
